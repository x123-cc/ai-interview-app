...
${questions.map((q, i) => `${i + 1}. ${q}`).join('\n')}`;
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-16">
      ...
      {/* 评分结果（仅完整面试有） */}
      {record.score != null && record.dimensions && (
        <div className="apple-card mb-10 p-6">
          <h2 className="mb-4 text-[1.125rem] font-semibold tracking-tight text-[#1d1d1f]">面试评分</h2>
          <div className="flex items-center gap-6 mb-5">
            <div className="text-center">
              <span className="text-[3rem] font-bold tracking-[-0.04em] text-[#1d1d1f] leading-none">{record.score}</span>
              <span className="text-[0.875rem] text-[#86868b]">/10</span>
            </div>
            <div className="flex-1 space-y-2">
              {Object.entries(record.dimensions).map(([name, score]) => {
                const pct = (score / 10) * 100;
                return (
                  <div key={name} className="flex items-center gap-2">
                    <span className="w-20 text-[0.75rem] font-medium text-[#1d1d1f]">{name}</span>
                    <div className="flex-1 h-1 rounded-full bg-[#e8e8ed] overflow-hidden">
                      <div
                        className="h-full rounded-full"
                        style={{
                          width: `${pct}%`,
                          backgroundColor: score >= 8 ? '#34c759' : score >= 6 ? '#ff9500' : '#ff3b30',
                        }}
                      />
                    </div>
                    <span className="w-5 text-right text-[0.75rem] font-semibold tabular-nums text-[#1d1d1f]">{score}</span>
                  </div>
                );
              })}
            </div>
